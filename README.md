...
This project uses the **Vehicle Dataset from CarDekho** ([Kaggle](https://www.kaggle.com/datasets/nehalbirla/vehicle-dataset-from-cardekho)), specifically `Car details v3.csv`. The dataset was collected around 2020 and reflects Indian used-car market listings (prices in INR). Data is not included in this repository — download it directly from Kaggle if you wish to reproduce the notebook.
...
![km_driven distribution after cleaning](images/km_driven_dist_cleaned.png)

With the extreme outliers removed, the distribution's real shape becomes visible: the bulk of listings fall under 150,000 km, with a long but much more reasonable right tail.
